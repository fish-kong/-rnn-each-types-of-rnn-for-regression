...
import pandas as pd
from nlstm_cell import NLSTMCell
...
def RNN(x):
    ...
    # 3 nLSTM with hidden number of nodes each layer
    lstm_cell1 = NLSTMCell(num_units=hidden_nodes)
    lstm_cell2 = NLSTMCell(num_units=hidden_nodes)
    lstm_cell3 = NLSTMCell(num_units=hidden_nodes)

    #stack of those layers
    lstm_cell = tf.nn.rnn_cell.MultiRNNCell([lstm_cell1, lstm_cell2, lstm_cell3])
#    lstm_cell = tf.nn.rnn_cell.MultiRNNCell([lstm_cell3])#only use one nlstm units
